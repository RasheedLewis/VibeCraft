# VibeCraft v2 Development Guide

This guide covers local development setup and deployment of VibeCraft v2.
...
For local development, you need to create an S3 bucket to store uploaded audio files.
...
1. **Check your AWS region** (from your AWS config):
   ```bash
...
2. **Create the S3 bucket**:
   ```bash
   # Default bucket name (can be changed in .env)
   aws s3 mb s3://vibecraft-videos --region <your-region>
   
   # Example for us-east-2:
   aws s3 mb s3://vibecraft-videos --region us-east-2
   ```

3. **Verify the bucket was created**:
   ```bash
   aws s3 ls | grep vibecraft-videos
   ```

4. **Configure bucket settings** (optional, for production):
   - Block public access: Enabled (we use presigned URLs, so public access isn't needed)
   - Versioning: Optional
   - Lifecycle policies: Optional (for cost optimization)

#### AWS Credentials

The application will automatically use AWS credentials from:
1. `.env` file (`AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`)
2. Environment variables
3. AWS CLI configuration (`~/.aws/credentials` from `aws configure`)
4. IAM roles (if running on EC2/ECS/Lambda)

**For local development**, you can use `aws configure` to set up credentials, and the application will automatically use them. No need to add them to `.env` unless you want to override.

#### Update .env (Optional)

If you want to explicitly set the bucket name or region in `.env`:
```bash
S3_BUCKET_NAME=vibecraft-videos
AWS_REGION=us-east-2  # Match your AWS CLI region
# AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY are optional if using aws configure
```
