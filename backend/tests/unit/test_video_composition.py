...
class TestTrimClipToBeatBoundary:
    """Tests for trim_clip_to_beat_boundary function.
    
    Justification: This function is critical for Phase 3.3 beat alignment.
    It ensures clips are trimmed to align with beat boundaries, which is
    essential for structural sync. Testing validates the trim calculation
    logic and FFmpeg filter generation.
    """

    @patch("app.services.video_composition.ffmpeg")
    def test_trim_to_beat_boundary_success(self, mock_ffmpeg):
        """Test successful trimming to beat boundary."""
        # Mock FFmpeg
        mock_input = Mock()
        mock_output = Mock()
        mock_overwrite = Mock()
        ...
        # Trim from 0-5s to beat-aligned 0.1-4.9s
        trim_clip_to_beat_boundary(
            clip_path="input.mp4",
            output_path="output.mp4",
        ...
        # Verify trim filter is used
        assert "trim=start=" in call_args[1]["vf"]
        assert "setpts=PTS-STARTPTS" in call_args[1]["vf"]

    @patch("app.services.video_composition.ffmpeg")
    def test_trim_calculates_correct_offsets(self, mock_ffmpeg):
        """Test that trim offsets are calculated correctly."""
        mock_input = Mock()
        mock_output = Mock()
        mock_overwrite = Mock()
        mock_ffmpeg.input.return_value = mock_input
        ...
        # Target: 0-10s, Beat: 0.5-9.5s
        # Should trim 0.5s from start and 0.5s from end
        trim_clip_to_beat_boundary(
            clip_path="input.mp4",
            output_path="output.mp4",
            target_start_time=0.0,
        ...
        call_args = mock_input.output.call_args
        filter_str = call_args[1]["vf"]
        
        # Should trim from 0.5 to 9.5 (beat_end_time - target_start_time = 9.5 - 0.0 = 9.5)
        assert "trim=start=0.500" in filter_str or "trim=start=0.5" in filter_str
        assert "end=9.500" in filter_str or "end=9.5" in filter_str

    @patch("app.services.video_composition.ffmpeg")
    def test_trim_handles_no_trim_needed(self, mock_ffmpeg):
        """Test trimming when clip is already aligned (no trim needed)."""
        mock_input = Mock()
        mock_output = Mock()
        mock_overwrite = Mock()
        mock_ffmpeg.input.return_value = mock_input
        ...
        # Target and beat times are the same - should still create filter
        trim_clip_to_beat_boundary(
            clip_path="input.mp4",
            output_path="output.mp4",
            target_start_time=0.0,
        ...
        mock_ffmpeg.input.assert_called_once()
        call_args = mock_input.output.call_args
        # Should still have trim filter (trim=start=0:end=5)
        assert "trim=" in call_args[1]["vf"]

    @patch("app.services.video_composition.ffmpeg")
    def test_trim_raises_error_on_ffmpeg_failure(self, mock_ffmpeg):
        """Test that trimming raises RuntimeError on FFmpeg failure."""
        # Create a mock exception that mimics ffmpeg.Error
        class MockFFmpegError(Exception):
            def __init__(self, cmd, stdout, stderr):
                self.cmd = cmd
        ...
        # Mock FFmpeg error
        error = MockFFmpegError("ffmpeg", b"Error message", b"stderr output")
        mock_overwrite.run.side_effect = error

        # Patch ffmpeg.Error to be our mock exception
        with patch("app.services.video_composition.ffmpeg.Error", MockFFmpegError):
            with pytest.raises(RuntimeError, match="Failed to trim clip to beat boundary"):
                trim_clip_to_beat_boundary(
                    clip_path="input.mp4",
                    output_path="output.mp4",
                    target_start_time=0.0,
...
class TestExtendClipToBeatBoundary:
    """Tests for extend_clip_to_beat_boundary function.
    
    Justification: This function extends clips to beat boundaries using frame
    freeze and fadeout. It's critical for Phase 3.3 to ensure clips align
    with musical beats. Testing validates extension calculation and filter
    generation.
    """

    @patch("app.services.video_composition.ffmpeg")
    def test_extend_to_beat_boundary_success(self, mock_ffmpeg):
        """Test successful extension to beat boundary."""
        mock_input = Mock()
        mock_output = Mock()
        mock_overwrite = Mock()
        mock_ffmpeg.input.return_value = mock_input
        ...
        # Extend from 4.5s to 5.0s beat boundary
        extend_clip_to_beat_boundary(
            clip_path="input.mp4",
            output_path="output.mp4",
            target_duration=4.5,
        ...
        # Verify tpad (extension) and fade filters are used
        assert "tpad=stop_mode=clone" in call_args[1]["vf"]
        assert "fade=t=out" in call_args[1]["vf"]
        assert "stop_duration=0.500" in call_args[1]["vf"] or "stop_duration=0.5" in call_args[1]["vf"]

    @patch("app.services.video_composition.trim_last_clip")
    @patch("app.services.video_composition.ffmpeg")
    def test_extend_uses_trim_when_no_extension_needed(self, mock_ffmpeg, mock_trim):
        """Test that trim is used when clip is already long enough."""
        # Clip is 5.0s, beat boundary is 4.9s - no extension needed
        extend_clip_to_beat_boundary(
            clip_path="input.mp4",
            output_path="output.mp4",
            target_duration=5.0,
        ...
        # Should call trim_last_clip instead of extending
        mock_trim.assert_called_once()
        mock_ffmpeg.input.assert_not_called()

    @patch("app.services.video_composition.ffmpeg")
    def test_extend_calculates_correct_duration(self, mock_ffmpeg):
        """Test that extension duration is calculated correctly."""
        mock_input = Mock()
        mock_output = Mock()
        mock_overwrite = Mock()
        mock_ffmpeg.input.return_value = mock_input
        ...
        # Extend from 3.0s to 5.0s = 2.0s extension
        extend_clip_to_beat_boundary(
            clip_path="input.mp4",
            output_path="output.mp4",
            target_duration=3.0,
        ...
        # Should extend by 2.0 seconds
        assert "stop_duration=2.000" in filter_str or "stop_duration=2.0" in filter_str
        # Fadeout should start at 4.5s (5.0 - 0.5)
        assert "st=4.500" in filter_str or "st=4.5" in filter_str

    @patch("app.services.video_composition.ffmpeg")
    def test_extend_with_custom_fadeout_duration(self, mock_ffmpeg):
        """Test extension with custom fadeout duration."""
        mock_input = Mock()
        mock_output = Mock()
        mock_overwrite = Mock()
        mock_ffmpeg.input.return_value = mock_input
        ...
        extend_clip_to_beat_boundary(
            ...
            target_duration=4.0,
            beat_end_time=5.0,
            fadeout_duration=1.0,  # Longer fadeout
        )

        call_args = mock_input.output.call_args
        filter_str = call_args[1]["vf"]
        
        # Fadeout should start at 4.0s (5.0 - 1.0) with 1.0s duration
        assert "st=4.000" in filter_str or "st=4.0" in filter_str
        assert "d=1.000" in filter_str or "d=1.0" in filter_str

    @patch("app.services.video_composition.ffmpeg")
    def test_extend_raises_error_on_ffmpeg_failure(self, mock_ffmpeg):
        """Test that extension raises RuntimeError on FFmpeg failure."""
        # Create a mock exception that mimics ffmpeg.Error
        class MockFFmpegError(Exception):
            def __init__(self, cmd, stdout, stderr):
                self.cmd = cmd
        ...
        # Patch ffmpeg.Error to be our mock exception
        with patch("app.services.video_composition.ffmpeg.Error", MockFFmpegError):
            with pytest.raises(RuntimeError, match="Failed to extend clip to beat boundary"):
                extend_clip_to_beat_boundary(
                    clip_path="input.mp4",
                    output_path="output.mp4",
                    target_duration=4.0,
